...
export function Sidebar() {
  ...
  async function handleNewSession() {
    if (isWingsightUserMode) {
      try {
        const session = await api.createSession({ backend: "codex", permissionMode: "bypassPermissions" });
        const name = `wingsight_montage 视频助手 ${new Date().toLocaleString("zh-CN", { hour12: false })}`;
        await api.renameSession(session.sessionId, name).catch(() => {});
    ...
    <aside aria-label="Session sidebar" className="w-full md:w-[260px] h-full flex flex-col bg-cc-sidebar">
      ...
      <div className="p-3.5 pb-2">
        <div className="flex items-center gap-2.5">
          {!isWingsightUserMode && <img src={logoSrc} alt="" className="w-6 h-6" />}
          <span className="text-[13px] font-semibold text-cc-fg tracking-tight">
            {isWingsightUserMode ? "WingSight_Agent" : "The Companion"}
          </span>
          <button
      ...
      {/* Footer */}
      {!isWingsightUserMode && <div className="px-2 py-1.5 pb-safe bg-cc-sidebar-footer border-t border-cc-border/30">
