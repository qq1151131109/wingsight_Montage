...
function SubagentContainer({ group }: { group: SubagentGroup }) {
  if (isWingsightUserMode) {
    return <UserModeSubtaskStatus group={group} />;
  }
...
function UserModeSubtaskStatus({ group }: { group: SubagentGroup }) {
  const status = normalizeSubagentStatus(group.status);
  const label = group.description || "子任务";
  const statusText =
    status?.summaryLabel === "completed" ? "已完成" :
    status?.summaryLabel === "failed" ? "失败" :
    status?.summaryLabel === "pending" ? "排队中" :
    "处理中";

  return (
    <div className="ml-10 flex items-center gap-2 py-1 text-[12px] text-cc-muted">
      <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
        status?.summaryLabel === "failed"
          ? "bg-cc-error"
          : status?.summaryLabel === "completed"
            ? "bg-cc-success"
            : "bg-cc-primary animate-[typing-breathe_1.5s_ease-in-out_infinite]"
      }`} />
      <span className="truncate">{label}</span>
      <span className="shrink-0 text-cc-muted/70">{statusText}</span>
    </div>
  );
}
        ...
        <div className="max-w-3xl mx-auto space-y-5 sm:space-y-7">
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-medium text-cc-fg">
                    {isWingsightUserMode ? "历史会话" : `${resumeModeLabel} existing Claude thread`}
                  </p>
          ...
          {canLoadResumeHistory && resumeHistoryLoaded && (
            <div className="flex justify-center">
              <p className="text-[11px] text-cc-muted">
                {resumeHistoryHasMore
                  ? resumeHistoryLoading
                    ? isWingsightUserMode ? "正在加载更早记录..." : "Loading older transcript..."
                    : isWingsightUserMode ? "滚动到顶部加载更早记录" : "Scroll to top to load older transcript"
                  : isWingsightUserMode ? "已加载全部历史记录" : "Loaded all available prior transcript"}
              </p>
            </div>
          )}
          ...
          {hasMore && (
            <div className="flex justify-center pb-3">
              ...
              >
                ...
                {isWingsightUserMode ? `加载更早 ${Math.min(FEED_PAGE_SIZE, hiddenCount)} 条` : `Load ${Math.min(FEED_PAGE_SIZE, hiddenCount)} more`}
                <span className="text-cc-muted/50 tabular-nums">
                  {isWingsightUserMode ? `（剩余 ${hiddenCount} 条）` : `(${hiddenCount} hidden)`}
                </span>
              </button>
            </div>
          )}
          ...
          {sessionStatus === "compacting" && (
            <div className="flex items-center gap-2 text-[11px] text-cc-warning font-mono-code pl-10 py-1">
              ...
              <span>{isWingsightUserMode ? "正在整理上下文..." : "Compacting context..."}</span>
            </div>
          )}
          ...
          {sessionStatus === "running" && elapsed > 0 && (
            <div className="flex items-center gap-2 text-[11px] text-cc-muted font-mono-code pl-10 stats-glow py-1">
              ...
              <span className="text-cc-fg/70">{isWingsightUserMode ? "处理中" : "Generating"}</span>
