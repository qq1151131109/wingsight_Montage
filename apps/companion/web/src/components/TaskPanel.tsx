...
export function TaskPanel({ sessionId }: { sessionId: string }) {
  ...
  const applicableSections = config.order.filter((sectionId) => {
    const def = SECTION_DEFINITIONS.find((d) => d.id === sectionId);
    if (!def) return false;
    if (isWingsightUserMode && !["project", "user"].includes(sectionId)) return false;
    if (def.backends && !def.backends.includes(backendFilter)) return false;
    return true;
  });
      ...
      {configMode ? (
        ...
        <>
          <div
            ...
            className={isWingsightUserMode
              ? "min-h-0 flex-1 flex flex-col overflow-hidden"
              : "min-h-0 flex-1 overflow-y-auto"}
          >
            {isWingsightUserMode ? (
              <div className="min-h-0 flex-1 border-b border-cc-separator">
          ...
          {!isWingsightUserMode && <div className="shrink-0 px-4 py-2 pb-safe border-t border-cc-separator">
            <button
