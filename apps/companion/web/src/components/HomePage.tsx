...
export function HomePage() {
  ...
  const [backend, setBackend] = useState<BackendType>(() =>
    isWingsightUserMode ? "codex" : ((localStorage.getItem("cc-backend") as BackendType) || "claude"),
  );
  ...
  const [selectedEnv, setSelectedEnv] = useState(() => isWingsightUserMode ? "" : (localStorage.getItem("cc-selected-env") || ""));
  const [showEnvDropdown, setShowEnvDropdown] = useState(false);
  const [showEnvManager, setShowEnvManager] = useState(false);

  // Sandbox state
  const [sandboxEnabled, setSandboxEnabled] = useState(() => isWingsightUserMode ? false : localStorage.getItem("cc-sandbox-enabled") === "true");
  const [sandboxes, setSandboxes] = useState<CompanionSandbox[]>([]);
  const [selectedSandbox, setSelectedSandbox] = useState(() => isWingsightUserMode ? "" : (localStorage.getItem("cc-selected-sandbox") || ""));
  ...
  useEffect(() => {
    if (isWingsightUserMode) {
      setBackend("codex");
      setModel(getDefaultModel("codex"));
      ...
      api.getProjectRoot().then(({ projectCwd }) => {
        setCwd(projectCwd);
      }).catch(() => {});
      return;
    }
  ...
  async function doCreateSession(
    ...
    store.clearCreation();
    const launchBackend = isWingsightUserMode ? "codex" : backend;
    const launchModel = isWingsightUserMode ? getDefaultModel("codex") : model;
    const launchMode = isWingsightUserMode ? getDefaultMode("codex") : mode;
    store.setSessionCreating(true, launchBackend as "claude" | "codex");
    ...
    try {
      ...
      const effectiveBranch = isWingsightUserMode
        ? undefined
        : launchOverride?.branch !== undefined
        ? (launchOverride.branch.trim() || undefined)
        : (selectedBranch.trim() || undefined);
      const effectiveUseWorktree = isWingsightUserMode
        ? false
        : launchOverride?.useWorktree !== undefined
      ...
      const result = await createSessionStream(
        {
          ...
          envSlug: isWingsightUserMode ? undefined : (selectedEnv || undefined),
          sandboxEnabled: isWingsightUserMode ? undefined : (sandboxEnabled ? true : undefined),
          sandboxSlug: isWingsightUserMode ? undefined : (sandboxEnabled && selectedSandbox ? selectedSandbox : undefined),
          branch: effectiveBranch,
          createBranch: effectiveCreateBranch ? true : undefined,
          useWorktree: effectiveUseWorktree ? true : undefined,
          backend: launchBackend,
          codexInternetAccess: launchBackend === "codex" ? true : undefined,
          resumeSessionAt: isWingsightUserMode ? undefined : effectiveResumeSessionAt,
          forkSession: isWingsightUserMode ? undefined : effectiveForkSession,
      ...
      sessionStore.setSdkSessions([
        ...existingSdkSessions,
        {
          ...
          cwd: result.cwd,
          createdAt: Date.now(),
          backendType: (result.backendType as BackendType | undefined) || launchBackend,
          model: launchModel,
          permissionMode: launchMode,
      ...
      useStore.getState().setPreviousPermissionMode(sessionId, launchMode);
  ...
  return (
    <div className="flex-1 h-full flex flex-col items-center px-3 sm:px-6 pb-6 pb-safe overflow-y-auto overscroll-y-contain">
      ...
      <div className="w-full max-w-[720px]">
        {/* Logo + Title — hidden in Wingsight user mode to avoid exposing Companion internals */}
        {!isWingsightUserMode && (
        ...
        <div className="relative bg-cc-card border border-cc-border rounded-2xl shadow-sm">
          ...
          <textarea
            ...
            placeholder={isWingsightUserMode ? "输入你的需求，上传参考图后也可以直接描述想做什么..." : "Fix a bug, build a feature, refactor code..."}
            rows={3}
        ...
        <div className="mt-3 sm:mt-4 space-y-2">
          ...
          {/* Backend toggle */}
          {!isWingsightUserMode && backends.length > 1 && (
          ...
          {!isWingsightUserMode && backend === "claude" && (
            <div>
              <button
                type="button"
            ...
            {!isWingsightUserMode && showOnboardingTip && (
