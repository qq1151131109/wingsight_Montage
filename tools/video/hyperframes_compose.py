...
class HyperFramesCompose(BaseTool):
    ...
    def _scaffold(self, inputs: dict[str, Any]) -> ToolResult:
        """Materialize the HyperFrames workspace from wingsight_montage artifacts.
        ...
        html = self._generate_index_html(
            ...
            or f"wingsight_montage {edit_decisions.get('renderer_family', 'composition')}",
        )
    ...
    def _style_bridge(
        ...
        """Bridge wingsight_montage playbook → HyperFrames CSS vars + DESIGN.md.
        ...
        design_md = (
            "# DESIGN\n\n"
            "Generated by wingsight_montage HyperFrames style bridge (fallback).\n\n"
            f"- Background: `{bg}`\n"
            f"- Foreground: `{fg}`\n"
            f"- Accent: `{accent}`\n"
            f"- Primary: `{primary}`\n"
