...
export function MessageBubble({ message }: { message: ChatMessage }) {
  if (message.role === "system") {
    if (isWingsightUserMode && shouldHideUserModeSystemMessage(message.content)) {
      return null;
    }
    return (
...
function ContentBlockRenderer({
  ...
  if (block.type === "tool_use") {
    if (isWingsightUserMode) return null;
  ...
  if (block.type === "tool_result") {
    ...
    if (isWingsightUserMode) {
      ...
      return <UserModeToolErrorBlock toolName={toolName} text={content} />;
    }
    if (toolName === "Bash") {
...
function shouldHideUserModeSystemMessage(content: string): boolean {
  const text = content.trim();
  return /protocol drift|unsupported incoming notification|Companion may need an update/i.test(text);
}

function UserModeToolErrorBlock({ toolName, text }: { toolName?: string; text: string }) {
  const [open, setOpen] = useState(false);
  const label = toolName ? getUserModeToolLabel(toolName) : "处理";

  return (
    <div className="rounded-lg border border-cc-error/20 bg-cc-error/5 overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-2 px-3 py-2 text-left text-[12px] text-cc-error hover:bg-cc-error/10 transition-colors cursor-pointer"
      >
        <svg
          viewBox="0 0 16 16"
          fill="currentColor"
        ...
        <span className="font-medium">{label}遇到问题</span>
        <span className="text-cc-error/70">点开查看技术详情</span>
      </button>
      {open && (
        <pre className="border-t border-cc-error/15 px-3 py-2 text-[11px] font-mono-code text-cc-error/80 whitespace-pre-wrap leading-relaxed max-h-60 overflow-y-auto">
          {text}
        </pre>
      )}
    </div>
...
function getUserModeToolLabel(name: string): string {
  if (name === "Bash") return "运行任务";
  if (name === "Read") return "读取文件";
  if (name === "Write" || name === "Edit" || name === "NotebookEdit") return "更新文件";
  if (name === "Glob" || name === "Grep") return "查找素材";
  if (name === "WebSearch" || name === "WebFetch" || name === "web_search") return "联网查询";
