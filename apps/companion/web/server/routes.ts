...
export function createRoutes(
  ...
  api.post("/sessions/create", async (c) => {
    ...
    if (isWingsightUserMode()) {
      ...
      if (projectDir) {
        body.cwd = projectDir;
        body.backend = body.backend || "codex";
        if (body.backend === "codex") {
          body.model = body.model || "gpt-5.5";
        }
        body.permissionMode = body.permissionMode || "bypassPermissions";
      }
    }
  ...
  api.get("/sessions", (c) => {
    ...
    if (isWingsightUserMode()) {
      const projectDir = getWingsightProjectDir();
      if (projectDir) {
        return c.json(enriched.filter((s) => {
  ...
  const wingsightProjectDir = getWingsightProjectDir();
  registerFsRoutes(
    api,
    isWingsightUserMode() && wingsightProjectDir
      ? { allowedBases: [wingsightProjectDir] }
      : undefined,
  );
