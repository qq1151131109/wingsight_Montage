...
function isTechnicalProtocolMessage(message: string): boolean {
  return /protocol drift|unsupported incoming notification|Companion may need an update/i.test(message);
}
...
function handleParsedMessage(
  ...
  switch (data.type) {
    ...
    case "error": {
      if (isWingsightUserMode && isTechnicalProtocolMessage(data.message)) {
        break;
      }
      store.appendMessage(sessionId, {
        id: nextId(),
