...
function prependProjectVirtualEnv(paths: string[]): string[] {
  const venv = process.env.VIRTUAL_ENV;
  if (!venv) return paths;

  const binDir = join(venv, process.platform === "win32" ? "Scripts" : "bin");
  if (!existsSync(binDir)) return paths;

  return [binDir, ...paths.filter((dir) => dir !== binDir)];
}
...
export function getEnrichedPath(): string {
  ...
  const allDirs = prependProjectVirtualEnv([
    ...userPath.split(pathSep),
