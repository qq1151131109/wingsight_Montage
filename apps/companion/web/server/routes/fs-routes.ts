...
const OFFICE_PREVIEW_LIMIT_BYTES = 100 * 1024 * 1024;
...
function shouldHideFromUserMode(name: string, isDirectory: boolean): boolean {
  if (!isWingsightUserMode()) return false;
  if (name.startsWith(".")) return true;
  if (isDirectory && TECHNICAL_DIRS.has(name)) return true;
  if (!isDirectory && TECHNICAL_FILES.has(name)) return true;
  return false;
}
...
async function previewWordDocument(absPath: string): Promise<PreviewResponse> {
...
function readSpreadsheet(buffer: Buffer, ext: string): XLSX.WorkBook {
  if (ext === "csv") {
    return XLSX.read(buffer.toString("utf-8"), { type: "string" });
  }
  return XLSX.read(buffer, { type: "buffer" });
}

async function previewSpreadsheet(absPath: string, ext: string): Promise<PreviewResponse> {
  const buffer = await readFile(absPath);
  const workbook = readSpreadsheet(buffer, ext);
  const sheets = workbook.SheetNames.slice(0, 8).map((name) => {
    ...
    const previewRows = rows
      ...
      .map((row) => row.slice(0, SPREADSHEET_PREVIEW_COLUMNS).map((cell) => cell ?? ""));
  ...
  const truncated = sheets.some(
    (sheet) =>
      sheet.rowCount > SPREADSHEET_PREVIEW_ROWS ||
      sheet.columnCount > SPREADSHEET_PREVIEW_COLUMNS,
  );
...
export function registerFsRoutes(api: Hono, opts?: { allowedBases?: string[] }): void {
  ...
  const projectDir = () => resolve(process.env.WINGSIGHT_MONTAGE_PROJECT_DIR || process.cwd());
  const userFilesRoot = () => resolve(projectDir(), "projects");
  const uploadsDir = () => resolve(userFilesRoot(), "uploads");
  ...
  api.get("/fs/project-root", (c) => {
    const cwd = isWingsightUserMode() ? userFilesRoot() : projectDir();
    return c.json({
      cwd,
  ...
  api.get("/fs/tree", async (c) => {
    ...
    async function buildTree(dir: string, depth: number): Promise<TreeNode[]> {
      ...
      try {
        ...
        for (const entry of entries) {
          if (shouldHideFromUserMode(entry.name, entry.isDirectory())) continue;
  ...
  api.get("/fs/preview", async (c) => {
    ...
    try {
      ...
      if (info.size > OFFICE_PREVIEW_LIMIT_BYTES) {
        return c.json({ error: "File too large for preview (>100MB)" }, 413);
      }

      const ext = getFileExtension(absPath);
      if (ext === "doc" || ext === "docx") {
        return c.json(await previewWordDocument(absPath));
      }
      if (ext === "xls" || ext === "xlsx" || ext === "csv") {
        return c.json(await previewSpreadsheet(absPath, ext));
      }

      return c.json({ error: "Preview is not supported for this file type" }, 415);
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : "Cannot preview file";
  ...
  api.get("/fs/raw", async (c) => {
    ...
    try {
      ...
      if (info.size > 100 * 1024 * 1024 * 1024) {
        return c.json({ error: "File too large (>100GB)" }, 413);
      }
    } catch (e: unknown) {
  ...
  api.post("/fs/upload", async (c) => {
    const uploadDirRaw = c.req.query("dir") || resolve(process.cwd(), "projects", "uploads");
    ...
    try {
      ...
      for (const file of files) {
        ...
        saved.push({
          name: safeName,
          path: guardedTarget,
          relPath: relative(process.env.WINGSIGHT_MONTAGE_PROJECT_DIR || process.cwd(), guardedTarget) || safeName,
          size: buffer.length,
        });
      }
