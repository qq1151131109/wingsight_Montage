...
export const api = {
  ...
  previewFile: (path: string) =>
    get<FilePreview>(
      `/fs/preview?path=${encodeURIComponent(path)}`,
    ),
  ...
  getProjectRoot: () =>
    get<{ cwd: string; projectCwd: string; uploads: string }>("/fs/project-root"),
  uploadFiles: async (dir: string, files: File[]) => {
    ...
    return res.json() as Promise<{ ok: boolean; files: Array<{ name: string; path: string; relPath: string; size: number }> }>;
  },
