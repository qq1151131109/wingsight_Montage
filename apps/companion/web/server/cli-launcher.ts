...
function withProjectVirtualEnv(env: Record<string, string | undefined>, cwd?: string): Record<string, string | undefined> {
  const projectDir = process.env.WINGSIGHT_MONTAGE_PROJECT_DIR || cwd;
  const venv = process.env.VIRTUAL_ENV || (projectDir ? join(projectDir, ".venv") : undefined);
  if (!venv) return env;
  ...
  if (!existsSync(python)) return env;

  const pathSep = process.platform === "win32" ? ";" : ":";
  const currentPath = env.PATH || "";
  const pathParts = currentPath.split(pathSep).filter((part) => part && part !== binDir);

  return {
    ...env,
    VIRTUAL_ENV: venv,
    PYTHON: env.PYTHON || python,
    PYTHON3: env.PYTHON3 || python,
    PIP: env.PIP || join(binDir, process.platform === "win32" ? "pip.exe" : "pip"),
    PATH: [binDir, ...pathParts].join(pathSep),
  };
}
...
export class CliLauncher {
  ...
  private spawnCLI(sessionId: string, info: SdkSessionInfo, options: LaunchOptions & { resumeSessionId?: string }): void {
    ...
    if (isContainerized) {
      ...
      spawnEnv = withProjectVirtualEnv({ ...process.env, PATH: getEnrichedPath() }, info.cwd);
      ...
      spawnEnv = withProjectVirtualEnv({
        ...process.env,
