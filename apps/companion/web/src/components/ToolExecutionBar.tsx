...
export function ToolExecutionBar({
  ...
  if (isWingsightUserMode) {
    return (
      <div
        className="flex items-center gap-2 text-[12px] text-cc-muted pl-10 py-1.5 animate-[fadeSlideIn_0.3s_ease-out]"
        role="status"
        aria-label="正在处理"
      >
        <span className="w-2 h-2 rounded-full bg-cc-primary animate-[typing-breathe_1.5s_ease-in-out_infinite]" />
        <span>正在处理</span>
      </div>
    );
  }
