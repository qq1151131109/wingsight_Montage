  ...
  /api/v3/contents/generations/tasks:
    get:
      ...
      description: >-
        ...
        对应 Seedance API: GET /api/v3/contents/generations/tasks

        <br/>

        仅支持查询最近 7
        天的历史数据。时间计算统一采用UTC时间戳，返回的7天历史数据范围以用户实际发起批量查询请求的时刻为基准（精确到秒），时间戳区间为 [T-7天,
        T)。
      ...
      parameters:
        ...
        - name: filter.task_ids
          in: query
          description: >-
            视频生成任务 ID，精确搜索，支持同时搜索多个任务 ID。多个任务 ID 之间通过
            &连接。示例：filter.task_ids=id1&filter.task_ids=id2。
          required: false
          schema:
            type: array
            items:
        ...
        - name: filter.model
          in: query
          description: 模型 ID。与返回参数不同，该字段为任务使用的推理接入点 ID，精确搜索。
          required: false
          schema:
            type: string
        - name: Authorization
      ...
      x-run-in-apifox: https://app.apifox.com/web/project/8016881/apis/api-438656208-run
components:
  schemas:
