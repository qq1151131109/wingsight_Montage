...
export function Composer({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    const onInsertText = (event: Event) => {
      ...
      const prefix = text && start > 0 && !/\s$/.test(text.slice(0, start)) ? " " : "";
      const nextText = text.slice(0, start) + prefix + detail.text + text.slice(end);
      const nextCursor = start + prefix.length + detail.text.length;
    ...
    window.addEventListener("companion:insert-text", onInsertText);
    return () => window.removeEventListener("companion:insert-text", onInsertText);
  }, [sessionId, text]);
  ...
  const deletePrompt = useCallback(async (prompt: SavedPrompt) => {
    try {
      await api.deletePrompt(prompt.id);
      await mention.refreshPrompts();
    } catch {
      // Keep the menu open; failing silently matches the existing prompt UI style.
    }
  }, [mention]);
  ...
  async function handleCreatePrompt() {
    ...
    const effectiveScope = isWingsightUserMode ? "project" : savePromptScope;
    if (!content || !name) return;
    if (effectiveScope === "project" && !sessionData?.cwd) {
        ...
        <div className={`relative overflow-visible transition-all duration-200 border-t border-cc-separator sm:border sm:border-cc-border sm:bg-cc-input-bg/95 sm:rounded-[16px] sm:backdrop-blur-sm composer-card ${
          ...
          {savePromptOpen && (
            <div className="absolute left-2 right-2 bottom-full mb-1 bg-cc-card border border-cc-border rounded-[10px] shadow-lg z-20 p-3 space-y-2">
              ...
              {!isWingsightUserMode && savePromptScope === "project" && sessionData?.cwd && (
          ...
          <div className="flex items-center gap-1.5 px-3 pt-1.5 pb-0.5 sm:hidden">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2 h-8 rounded-md text-[12px] transition-colors ${
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 pb-2">
            ...
            <button
              ...
              disabled={!isConnected || !text.trim()}
              className={`flex items-center gap-1.5 px-2.5 h-8 rounded-md text-[12px] font-medium transition-colors shrink-0 ${
                isConnected && text.trim()
                  ? "text-cc-muted hover:text-cc-fg hover:bg-cc-hover cursor-pointer"
                  : "text-cc-muted opacity-30 cursor-not-allowed"
              }`}
